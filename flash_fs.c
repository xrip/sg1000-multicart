...
unsigned long flash_fs_get_FAT_sector(uint16_t fat_sector)
{
    int mapEntry = fs_map.sectors[fat_sector];
    if (mapEntry) {
        //  printf("[FS] READ: %d, %d (%d)\n", sector, offset, size);
        const uint32_t fs_start = XIP_BASE + HW_FLASH_STORAGE_BASE;
        return fs_start + (getMapSector(mapEntry) * FLASH_SECTOR_SIZE) + (getMapOffset(mapEntry) * 512);
    }
